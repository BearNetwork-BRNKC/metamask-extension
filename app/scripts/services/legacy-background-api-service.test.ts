...
describe('LegacyBackgroundApiService', () => {
  ...
  describe('permission background API methods', () => {
    ...
    describe('addPermittedAccount', () => {
      ...
      it('invokes onPermittedAccountsAdded when a new CAIP account id is added', async () => {
        const onPermittedAccountsAdded = jest.fn();
        await withService(
          { options: { onPermittedAccountsAdded } },
          async ({ rootMessenger }) => {
            registerPermissionHandlers(rootMessenger, {
              getCaveat: jest.fn().mockReturnValue({
                value: {
      ...
      it('does not invoke onPermittedAccountsAdded when the account was already permitted', async () => {
        const onPermittedAccountsAdded = jest.fn();
        await withService(
          { options: { onPermittedAccountsAdded } },
          async ({ rootMessenger }) => {
...
async function withService<ReturnValue>(
  ...
  const service = new LegacyBackgroundApiService({
    ...
    onPermittedAccountsAdded: jest.fn(),
    ...options,
  });
